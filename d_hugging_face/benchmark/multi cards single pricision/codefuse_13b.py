...
def main():
    ...
    local_rank = int(os.environ.get("LOCAL_RANK", 0))
    result=benchmarkObject.get_benchmark_result().item()
    if local_rank==0:
        print("codefuse: {:.3f}token/s".format(result))

        with open("log_codefuse_7b.csv","a+") as csvfile: 
            writer = csv.writer(csvfile)

            writer.writerow([
                benchmark_config.dp_size,
                benchmark_config.max_input_length,
                benchmark_config.max_output_length,
                benchmark_config.batch_size,
                result,
                benchmark_config.tp_size,
                benchmark_config.pipeline_size
            ])
    dist.barrier()
